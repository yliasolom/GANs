...
class generator(nn.Module):
    ...
    def forward(self, input):
        # x = F.relu(self.deconv1(input)) - и так можно
...
# берем Leaky_relu instead usual Relu
    def forward(self, input):
...
def show_result(num_epoch, show = False, save = False, path = 'result.png', isFix=False):
    z = torch.randn((5*5, 100)).view(-1, 100, 1, 1)#расплющим
    ...
    for elem in range(5*5):
        i = elem // 5
        j = elem % 5
        ax[i, j].cla()
        ax[i, j].imshow((test_images[elem].data.numpy().transpose(1, 2, 0) + 1) / 2)
...
temp = plt.imread(train_loader.dataset.imgs[0][0])#временный
if (temp.shape[0] != img_size) or (temp.shape[0] != img_size):
    sys.stderr.write('Error:((((((! image size  not 64 x 64.... run \"data_preprocess.py\" !!!.....')
    sys.exit(1)
...
# Binary Cross Entropy loss - считаем ошибку
BCE_loss = nn.BCELoss()

# optimizer - берем самый популярный
...
print("Training finishhhh!... save training results")
